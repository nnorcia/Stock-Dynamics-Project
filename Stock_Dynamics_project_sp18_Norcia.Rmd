...
A stock market is where buyers and sellers trade shares of a company,  and is one of the most popular ways for individuals and companies to invest money. 
...
#I added MSFT to show you I can do code on my own
setwd("N:/School/3710/Ch11/stock_dynamics_project")
...
#
MSFT$Date = as.Date(MSFT$Date, "%m/%d/%y")
```



The first argument to the `as.Date` function is the variable we want to convert, and the second argument is the format of the Date variable.
...
```{r}
#The summary and structure of data set IBM
summary(IBM)
str(IBM)

#The summary and structure of data set GE
summary(GE)
str(GE)

#The summary and structure of data set CocaCola
summary(CocaCola)
str(CocaCola)

#The summary and structure of data set ProcterGamble
summary(ProcterGamble)
str(ProcterGamble)
sd(ProcterGamble$StockPrice)

#The summary and structure of data set Boeing
summary(Boeing)
str(Boeing)

#The summary and structure of data set MSFT
summary(MSFT)
str(MSFT)
```
...
You should see the plot for Coca-Cola show up again, but this time in red. Now, let's add the Procter & Gamble line (using the lines function like we did before), adding the argument `col="blue"`. 
...
Ans: Proctor Gamble dropped more.
...
Ans:  CocaCola, I cannot really tell without altering the code.
...
Let's take a look at how the stock prices changed from 1995-2005 for all five companies. Use following plot command:
...
Now, use the lines function to add in the other four companies, remembering to only plot the observations from 1995 to 2005, or [301:432]. 
You don't need the `"type" or "ylim"` arguments for the lines function, but remember to make each company a different color so that you can tell them apart. 
Some color options are `"red", "blue", "green", "purple", "orange", and "black"` . To see all of the color options in R, type `colors()` in your R console.
...
`lty=4` will make the line alternate between dashes and dots, and 
...
Ans:  April, February, January, March, and May
